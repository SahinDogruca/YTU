...
void printShortestPath(char grid[ROW][COL], int startRow, int startCol, int destRow, int destCol) {
    ...
    char foundChars[ROW * COL]; // Bulunan karakterleri saklamak için
    ...
    while (!isEmpty(queue)) {
        ...
        if (isDestination(row, col, destRow, destCol)) {
            ...
            // Bulunan karakterleri yazdır
            printf("Bulunan karakterler: ");
            for (int i = foundCharsCount - 1; i >= 0; i--) {
                printf("%c ", foundChars[i]);
            }
            printf("\n");
        ...
        // Eğer e, E, p, P karakterleri bulunursa diziye ekle
        if ((grid[row][col] == 'e' || grid[row][col] == 'E' || grid[row][col] == 'p' || grid[row][col] == 'P') &&
            (foundCharsCount == 0 || foundChars[foundCharsCount - 1] != grid[row][col])) {
            foundChars[foundCharsCount++] = grid[row][col];
        }
